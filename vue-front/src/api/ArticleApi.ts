...
export const getArticleListApi = <T>(
  pageSize: number,
  currentPage: number,
  searchText = ""
): Promise<ReturnPageType<T>> =>
  request.get(
    `/article/getArticleList?pageSize=${pageSize}&currentPage=${currentPage}&searchText=${searchText}`
  );
export const getArticleByIdApi = <T>(id:number):Promise<ReturnType<T>> =>request.get(`/article/getArticleById/${id}`)